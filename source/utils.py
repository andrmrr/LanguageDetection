...
from sklearn.feature_extraction.text import CountVectorizer
from preprocess import get_symbol_freq_by_family
...
def compute_features(X_train, 
  ...
  Task: Compute a matrix of token counts given a corpus. 
        This matrix represents the frequency any pair of tokens appears
...
def add_generated_features(X_train_raw, X_test_raw, X_train, X_test):
    ...
    return np.concatenate((X_train_raw, train_array), axis=1), np.concatenate((X_test_raw, test_array), axis=1)
...
def get_periods_ratio(sentence):
    cnt = 0.0
    for character in sentence:
        if character == '.':
            cnt += 1.0
...
def get_special_symbols_ratio(sentence):
    punctuation_chars = "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"
    cnt = 0.0
    for character in sentence:
